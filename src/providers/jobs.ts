...
export class Jobs {
 ...
 //Gets the review data
  getJobs(){
    ...
    return new Promise(resolve => {
      ...
      this.http.get('https://kierantradie.herokuapp.com/api/reviews')
        .map(res => res.json())
        .subscribe(data => {
          this.data = data;
          resolve(this.data);
 ...
 //Here we are connecting to our hosted Node application containing our API so that app can post the data
    this.http.post('https://kierantradie.herokuapp.com/api/reviews', JSON.stringify(job), {headers: headers})
      .subscribe(res => {
 ...
 //Here we are connecting to our hosted Node application containing our API so that app can delete the data
    this.http.delete('https://kierantradie.herokuapp.com/api/reviews/' + id).subscribe((res) => {
      console.log(res.json());
    });    
